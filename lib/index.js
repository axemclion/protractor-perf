...
var ProtractorPerf = function(protractor) {
	this.sessionId = null;
	this.protractor = protractor;
	var conf = protractor.getInstance().params;
	if (conf.perf) {
		this.isEnabled = true;
...
ProtractorPerf.prototype.getSession_ = function() {
	if (this.sessionId) {
		...
		return this.protractor.getInstance().driver.getSession().then(function(session) {
...
function insideControlFlow(fn) {
	return function() {
		var args = arguments;
		return this.protractor.getInstance().controlFlow().execute(function() {
